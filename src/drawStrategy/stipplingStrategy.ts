...
function* stipplingPointProducer(edges:Float64Array) {
    let i=2;
    while(i<edges.length) {
        const sample = edges.slice(0, Math.min(i, edges.length));
        console.log('i', i);
        yield sample;
        i= i<<1;
    }
...
const transformCenteroid = generatorConsumer(function* ( receiver:Generator<Voronoi<number>>, imageData:ImageData, colorWeights:Float64Array) {
...
const drawDelaunayConsumer = generatorConsumer(function*(ctx:CanvasRenderingContext2D, imageData:ImageData){
